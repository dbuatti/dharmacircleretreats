...
import { Search, Filter, Plus, Trash2, CheckCircle2, Tag, Users, Loader2 } from "lucide-react";
import { toast } from "sonner";
import { getColumns } from "./sheet/columns";
import { ParticipantTable } from "./sheet/ParticipantTable"; // Import the new table component
...
const ParticipantSheetComponent: React.FC<ParticipantSheetProps> = ({
  ...
  const columns = useMemo(() => {
    return getColumns({ onDeleteParticipant });
  }, [onDeleteParticipant]);
  ...
  const table = useReactTable({
    ...
    getRowId: (row) => row.id, // Ensure stable row IDs
    state: {
      sorting,
      globalFilter,
      rowSelection,
