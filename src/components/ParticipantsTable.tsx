...
export const ParticipantsTable: React.FC<ParticipantsTableProps> = ({
    ...
    try {
      ...
      toast.success(`${idsToUpdate.length} participants updated`, {
        action: {
          label: "Undo",
          onClick: () => {
            // In a real app, you'd store previous values to revert
      ...
      // Rollback logic would go here (revert UI to previous state)
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      // Rollback would restore deleted items
    }
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'Enter' && selectedIds.length > 0) {
        e.preventDefault();
        // Re-apply last bulk action (simulated)
