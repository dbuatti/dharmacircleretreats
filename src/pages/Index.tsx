...
const Index = () => {
  ...
  const handleAddParticipant = () => {
    ...
    const participant: Participant = {
      ...
      source: newParticipant.source || "manual",
      registration_status: newParticipant.registration_status || "not_sent",
      payment_status: newParticipant.payment_status || "not_paid",
      attendance_status: newParticipant.attendance_status || "interested",
      dietary_requirements: newParticipant.dietary_requirements || "",
      notes: newParticipant.notes || "",
      tags: newParticipant.tags || [],
    ...
    toast({ 
      ...
      description: `${participant.full_name} has been added to the retreat.` 
    });
  };
  ...
  const handleSendEmails = () => {
    ...
    toast({ 
      title: "Emails sent", 
      description: `${selected.length} participants received the ${emailTemplate} email.` 
    });

    // Update last contacted
    const updated = participants.map(p => 
      selectedParticipants.includes(p.id) 
        ? { ...p, last_contacted: new Date(), registration_status: p.registration_status === "not_sent" ? "sent" as const : p.registration_status }
        : p
    );
  ...
  const handleCopyWhatsApp = () => {
    ...
    toast({ title: "WhatsApp link copied", description: "Share with participants" });
  };
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          <Button 
                            ...
                            variant="ghost"
                            onClick={() => {
                              const updated = participants.map(part => 
                                part.id === p.id 
                                  ? { ...part, registration_status: part.registration_status === "confirmed" ? "incomplete" as const : "confirmed" as const }
                                  : part
                              );
                              setParticipants(updated);
                          ...
                          <Button 
                            ...
                            onClick={() => {
                              const updated = participants.map(part => 
                                part.id === p.id 
                                  ? { ...part, payment_status: part.payment_status === "paid_in_full" ? "not_paid" as const : "paid_in_full" as const }
                                  : part
                              );
                              setParticipants(updated);
